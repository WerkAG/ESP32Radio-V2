...
#include "src/utils.h"                                        // Some handy utilities
  ...
  #include "src/mp3_decoder.h"                                // Yes, include libhelix_HMP3DECODER
  #include "src/aac_decoder.h"                                // and libhelix_HAACDECODER
  #include "src/helixfuncs.h"                                 // Helix functions
#else
  #include "src/VS1053.h"                                     // Driver for VS1053
#endif
...
#define BL_TIME           300                             // Time-out [sec] for blanking TFT display (BL pin)
//
...
#ifdef BLUETFT
 #include "src/bluetft.h"                                        // For ILI9163C or ST7735S 128x160 display
#endif
#ifdef ST7789
 #include "src/ST7789.h"                                         // For ST7789 240x240 display
#endif
#ifdef ILI9341
 #include "src/ILI9341.h"                                        // For ILI9341 320x240 display
#endif
#ifdef OLED1306
 #include "src/oled.h"                                           // For OLED I2C SD1306 64x128 display
#endif
#ifdef OLED1309
 #include "src/oled.h"                                           // For OLED I2C SD1309 64x128 display
#endif
#ifdef OLED1106
 #include "src/oled.h"                                           // For OLED I2C SH1106 64x128 display
#endif
#ifdef LCD1602I2C
 #include "src/LCD1602.h"                                        // For LCD 1602 display (I2C)
#endif
#ifdef LCD2004I2C
 #include "src/LCD2004.h"                                        // For LCD 2004 display (I2C)
#endif
#ifdef DUMMYTFT
 #include "src/dummytft.h"                                       // For Dummy display
#endif
#ifdef NEXTION
 #include "src/NEXTION.h"                                        // For NEXTION display
#endif

// Include software for SD card.  Will include dummy if "SDCARD" is not defined
#include "src/SDcard.h"                                         // For SD card interface
...
void readIOprefs()
{
  ...
  struct iosetting klist[] = {                            // List of I/O related keys
    ...
    { "pin_tft_blx",   &ini_block.tft_blx_pin,      33 }, // Display backlight (inversed logic)
    { "pin_sd_cs",     &ini_block.sd_cs_pin,        13 }, // SD card select
    ...
    tripleclick = false ;                                     // Reset flag
      ...
      if ( SD_filecount )                                     // Tracks on SD?
      {
        ...
        tftset ( 3, "Turn to select track\n"                  // Show current option
                    "Press to confirm" ) ;
        getNextSDFileName() ;                                 // Start with next file on SD
      }
      else
      {
...
#ifdef SDCARD
    case TRACK :
      getSDFileName ( getCurrentSDTrackNumber() +            // Select next / previous file on SD
                      rotationcount ) ;
      dbgprint ( "Select track %d: %s",                      // Show for debug
                  getCurrentSDTrackNumber(),
...
//   track      = 34                        // Select MP3 track from SD card                       *
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  {
    if ( relative )                                   // Was it relative?
    {
      ivalue = getCurrentSDTrackNumber() + ivalue ;   // Yes, relative to current tracknumber
    }
    getSDFileName ( ivalue ) ;                        // Yes, select new track
    myQueueSend ( sdqueue, &startcmd ) ;              // Signal SDfuncs()
  }
#endif
